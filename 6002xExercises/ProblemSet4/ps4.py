...
def generate_models(x, y, degs):
    ...
    for degree in degs:
        ...
        models_coefficients.append(coefficients)
...
def r_squared(y, estimated):
    ...
    """
    mean_y = sum(y)/len(y)
    squared = sum((np.array(y)-np.array(estimated))**2) / sum((np.array(y)-mean_y)**2)
    r_squared = 1 - squared
    return r_squared
...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    pylab.figure("Linear Regression Evaluation")
    pylab.plot(x, y, "bo", label="Sampled Points")
    pylab.title("Evaluate Models of Linear Regression")
    pylab.xlabel("Years")
    ...
    for model in models:
        ...
        pylab.plot(x, estimated_y, label=("Fit with Degree "+str(degree)+". R^2="+str(round(r_2, 4))))
